...
export const getPlatformSupportedAudioSources = (): AudioSource[] => {
  ...
  return iOS() ? [AUDIO_SOURCE.FILE_UPLOAD, AUDIO_SOURCE.MICROPHONE] : [AUDIO_SOURCE.MICROPHONE, AUDIO_SOURCE.LIVE_STREAM, AUDIO_SOURCE.FILE_UPLOAD];
};
...
export function useSelectAudioSource() {
  ...
  const { audioSource } = useControls({
    Audio: folder({
      audioSource: {
        value: audioSourceParam && AVAILABLE_SOURCES.includes(audioSourceParam) ? audioSourceParam : AVAILABLE_SOURCES[0],
